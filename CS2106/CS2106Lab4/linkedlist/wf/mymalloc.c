...
void *mymalloc(size_t size) {
    ...
    TNode *worstFitPtr = NULL;
    long worstSizeSoFar = -999;
    ...
    while (ptr != NULL) {
        ...
        if (data->free && data->partitionSize >= size) {
            if (data->partitionSize == size) {
                ...
                if (data->partitionSize > worstSizeSoFar) {
                    worstFitPtr = ptr;
                    worstSizeSoFar = data->partitionSize;
                }
            }
        }
    ...
    if (worstFitPtr != NULL) {
        ...
        // Set the corresponding bits in _heap to 1
        for (long i = worstFitPtr->key; i < worstFitPtr->key + size; i++) {
            _heap[i] = 1;
        }
        ...
        // Return pointer to the allocated memory
        return (void *)(_heap + worstFitPtr->key);
