...
void merge_node(TNode *llist, TNode *node, int dir) {
    if (node == NULL || node->pdata == NULL)
        return;
    if(dir == 0) {
        if(node->prev == NULL || node->prev->pdata == NULL || node->pdata == NULL )
            return;
        
        long newSize = node->prev->pdata->partitionSize + node->pdata->partitionSize;
        node->prev->pdata->partitionSize = newSize;

        delete_node(&llist, node);
    }
    else 
        if(dir == 1) {
            if(node->next == NULL|| node->next->pdata == NULL || node->pdata == NULL )
                return;
            long newSize = node->next->pdata->partitionSize + node->pdata->partitionSize;
            node->pdata->partitionSize = newSize;
